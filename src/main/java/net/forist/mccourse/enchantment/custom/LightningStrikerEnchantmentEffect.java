package net.forist.mccourse.enchantment.custom;
...
public record LightningStrikerEnchantmentEffect() implements EnchantmentEntityEffect
{
    ...
    @Override
    public void apply(ServerLevel pLevel, int pEnchantmentLevel, EnchantedItemInUse enchantedItemInUse, Entity pEtity, Vec3 vec3)
    {
        if (pEnchantmentLevel == 1)
        {
            EntityType.LIGHTNING_BOLT.spawn(pLevel, pEtity.getOnPos(), MobSpawnType.TRIGGERED);
        }
